...
(test (make-closures (build-L5expr '(let ([x (lambda (y) (+ x y))]) x)))
      (cljmap 1
              (lft-clj ':l5clj_0
                       '(vars_tup y)
                       '(x)
                       (build-L5expr '(let ([x (aref vars_tup 0)]) (+ x y))))))
...
(test (compile-L5expr (build-L5expr '(let ([x (lambda (t) (t 4 5))]) (x z))))
      (build-L4prog '((let ([x (make-closure :l5clj_0 (new-tuple))])
                        ((closure-proc x) (closure-vars x) z))
                      (:l5clj_0 (vars_tup t)
                                ((closure-proc t) (closure-vars t) 4 5)))))
(test (compile-L5expr (build-L5expr '(let ([x (lambda (t) (t 4 5))]) (x +))))
      (build-L4prog '((let ([x (make-closure :l5clj_0 (new-tuple))])
                        (print ((closure-proc x) (closure-vars x) (make-closure :l5bif_plus (new-tuple)))))
                      (:l5clj_0 (vars_tup t)
                                ((closure-proc t) (closure-vars t) 4 5))
                      (:l5bif_plus (vars_tup arg1 arg2)
                                   (+ arg1 arg2)))))
